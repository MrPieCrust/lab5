...
/**
 * The Class CustomerArrives represents the event of a customer arriving to the store. When the performEvent 
 * is called the state is changed and a customer is created.
 * 
...
public class CustomerArrives extends Event {
	...
	private int custID;
/**
 * The first constructor assigns a time when the event is to be executed and adds it to the queue
 * 
 * @param state - The state of the simulation is sent in so that variables can be
 *            accessed from there.
 */
	...
	/**
	 * Does the same as previous constructor with the exception that the time to be executed
	 * is set to 0.
	 * 
	 * @param state - The state of the simulation is sent in so that variables can be
	 *            accessed from there.
	 * @param first - a dummy variable so that when the first customer arrives this constructor
	 * is called
	 */
